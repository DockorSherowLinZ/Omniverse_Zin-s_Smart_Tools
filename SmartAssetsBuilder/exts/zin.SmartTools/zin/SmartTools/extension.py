...
# Version: v1.7.4 (uniform log levels with colored UI: [INFO] / [WARN] / [ERROR])
#
# Source layout (*):
#   <Source Root>/<CORE>/max_<CORE>.usd
#   <Source Root>/<CORE>/Materials/...
#
# Output layout (*):
#   <Output Root>/id_<CORE>_<SUFFIX>.usd
#   <Output Root>/<CORE>/
#       ├─ max_<CORE>.usd           (copied here unless in-place; asset subLayers = [ ./max_<CORE>.usd ])
#       ├─ Materials/...            (recursively mirrored; supports cross-scheme copy; skipped in in-place)
#       ├─ asset_<CORE>.usd         (root-layer subLayers = [ ./max_<CORE>.usd ])
#       └─ <CORE>.usd               (/World/ASSET/asset_<CORE> references asset_<CORE>.usd)
...
def _build_asset(out_path: str, sublayer_target: str) -> str:
    ...
    rel = _dotify_rel(_relref(out_path, sublayer_target))
...
class Extension(omni.ext.IExt):
    def on_startup(self, ext_id):
        ...
        # Styles for log lines (fallback-safe)
        self._STYLE_INFO  = {"color": 0xFFB8D7FF}   # light blue
        self._STYLE_WARN  = {"color": 0xFFFFC857}   # amber
        self._STYLE_ERROR = {"color": 0xFFFF6B6B}   # red
        self._STYLE_HEAD  = {"font_size": 18}
    ...
    # ---------- UI ----------
    def _build_ui(self):
        self._window = ui.Window("SmartAssetsBuilder", width=860, height=880, dockPreference=ui.DockPreference.LEFT_BOTTOM)
        with self._window.frame:
            with ui.VStack(spacing=8, height=0):
                ui.Label("SmartAssetsBuilder (v1.7.4)", style=self._STYLE_HEAD)
                ui.Label("Source → Scan → List → Output Root → Start (copy max + Materials; build asset/main/id).")
                ui.Separator()

                ui.Label("Source Folder URL")
                self._folder_field = ui.StringField()

                with ui.HStack():
                    ui.Label("Filename filter")
                    self._filter_field = ui.StringField(); self._filter_field.model.set_value("max_*.usd")
                    ui.Spacer(width=16)
                    ui.Label("Recurse")
                    self._recurse_cb = ui.CheckBox(); self._recurse_cb.model.set_value(True)

                ui.Label("Output Root URL (local or omniverse://)")
                self._out_root_field = ui.StringField()

                with ui.HStack():
                    ui.Button("Scan", clicked_fn=self._on_scan)
                    ui.Spacer(width=12)
                    ui.Label("ID Suffix")
                    self._id_field = ui.StringField(); self._id_field.model.set_value("TEMP00000001")
                    ui.Spacer(width=12)
                    ui.Label("Overwrite")
                    self._overwrite_cb = ui.CheckBox(); self._overwrite_cb.model.set_value(False)

                with ui.HStack():
                    self._inplace_cb = ui.CheckBox(); self._inplace_cb.model.set_value(False)
                    ui.Label("Allow Same Root (in-place) — skips Materials copy to avoid recursion")

                ui.Label("Found files")
                with ui.ScrollingFrame(height=320):
                    self._list_label = ui.Label("")

                with ui.HStack():
                    ui.Button("Start (build trio)", clicked_fn=self._on_start)
                    ui.Spacer(width=16)
                    self._progress_label = ui.Label("Progress: 0/0")

                ui.Separator()
                ui.Label("Messages")
                # Colored log list
                self._log_frame = ui.ScrollingFrame(height=260)
                with self._log_frame:
                    self._log_stack = ui.VStack(spacing=2)

    # ---------- Logging (colored) ----------
    def _append_log_line(self, level: str, text: str):
        # Map level to style; swallow any style errors by falling back to no-style
        style = self._STYLE_INFO
        if level == "WARN":  style = self._STYLE_WARN
        if level == "ERROR": style = self._STYLE_ERROR
        try:
            with self._log_stack:
                ui.Label(f"[{level}] {text}", style=style)
        except Exception:
            with self._log_stack:
                ui.Label(f"[{level}] {text}")
        print(f"[SmartAssetsBuilder] [{level}] {text}")

    def _info(self, msg: str):  self._append_log_line("INFO", msg)
    def _warn(self, msg: str):  self._append_log_line("WARN", msg)
    def _error(self, msg: str): self._append_log_line("ERROR", msg)

    # Helper for functions that pass in pre-tagged strings (e.g., "[ERROR] ...")
    ...
    # ---------- UI actions ----------
    def _on_scan(self):
        # Reset log area by rebuilding the stack (simple, safe)
        try:
            with self._log_frame:
                self._log_stack = ui.VStack(spacing=2)
        except Exception:
        ...
        try:
            ...
            self._scan_root = url
            if not files:
                self._list_label.text = f"(No files matched '{pattern}' — {'recursive' if recurse else 'current level'})"
                self._warn(f"No files matched '{pattern}'")
            else:
                self._list_label.text = "\n".join(files)
                self._info(f"Found {len(files)} files")
        ...
        for i, src in enumerate(self._found, start=1):
            try:
                ...
                try:
                    ...
                    a_path = _build_asset(asset_path, max_dst)
